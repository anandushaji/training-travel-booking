import { Inject, Injectable, Logger } from '@nestjs/common';
import { NotFoundException, DomainException } from '@travel/shared';
import { PaymentStatus } from '../../../domain/value-objects/payment-status.enum';
import {
  IPaymentRepository,
  PAYMENT_REPOSITORY,
} from '../../../domain/repositories/payment.repository.interface';
import { StripeClientService } from '../../../infrastructure/stripe/stripe-client.service';
import { PaymentEventPublisher } from '../../../infrastructure/events/payment-event.publisher';
import { MetricsService } from '../../../infrastructure/observability/metrics.service';
import { ForbiddenException } from '../detach-payment-method/detach-payment-method.use-case';
import { CapturePaymentCommand, CapturePaymentResponseDto } from './capture-payment.command';

@Injectable()
export class CapturePaymentUseCase {
  private readonly logger = new Logger(CapturePaymentUseCase.name);

  constructor(
    @Inject(PAYMENT_REPOSITORY)
    private readonly paymentRepo: IPaymentRepository,
    private readonly stripeClient: StripeClientService,
    private readonly eventPublisher: PaymentEventPublisher,
    private readonly metrics: MetricsService,
  ) {}

  async execute(command: CapturePaymentCommand): Promise<CapturePaymentResponseDto> {
    const payment = await this.paymentRepo.findById(command.paymentId);
    if (payment === null) {
      throw new NotFoundException(
        `Payment ${command.paymentId} not found.`,
        { code: 'PAYMENT_NOT_FOUND' },
      );
    }

    // Ownership check BEFORE state check
    if (payment.travelerId !== command.callerTravelerId) {
      throw new ForbiddenException(
        `Payment ${command.paymentId} does not belong to the caller.`,
      );
    }

    // State check
    if (payment.status !== PaymentStatus.AUTHORIZED) {
      throw new DomainException(
        `Cannot capture payment in status ${payment.status}`,
        'INVALID_STATE_TRANSITION',
        409,
        { currentStatus: payment.status },
      );
    }

    // Full capture — no amountToCapture (OQ-3 resolved)
    const intent = await this.stripeClient.capturePaymentIntent(
      payment.stripePaymentIntentId!,
    );

    const capturedAmount =
      intent.amount_received !== undefined
        ? intent.amount_received / 100
        : payment.money.amount;

    payment.markCaptured(capturedAmount, command.correlationId);
    await this.paymentRepo.save(payment);
    this.metrics.incrementPaymentsCaptured(payment.money.currency);

    for (const event of payment.getUncommittedEvents()) {
      try {
        await this.eventPublisher.publish(event as any);
      } catch (err: unknown) {
        this.logger.warn(`Kafka publish failed for PaymentCaptured: ${String(err)}`);
      }
    }

    return {
      paymentId: payment.paymentId,
      status: payment.status,
      capturedAmount,
      currency: payment.money.currency,
    };
  }
}
