import { UpdateTravelerUseCase } from './update-traveler.use-case';
...
describe('UpdateTravelerUseCase', () => {
  it('should throw TravelerNotFoundException when traveler does not exist', async () => {
    ...
    const useCase = new UpdateTravelerUseCase(repository, cache, publisher);
    await expect(
      useCase.execute('unknown-id', { name: 'Bob' }),
    ).rejects.toBeInstanceOf(TravelerNotFoundException);
  });

  it('should invalidate cache after repository save', async () => {
    const { repository, cache, publisher } = makeMocks();
    ...
    const useCase = new UpdateTravelerUseCase(repository, cache, publisher);
    await useCase.execute('traveler-uuid-1', { name: 'Bob' });

    expect(repository.save).toHaveBeenCalled();
    expect(cache.invalidate).toHaveBeenCalledWith('traveler-uuid-1');
  });

  it('should publish TravelerUpdated event after save', async () => {
    const { repository, cache, publisher } = makeMocks();
    repository.findById.mockResolvedValue(buildTraveler());
    repository.save.mockResolvedValue(undefined);
    cache.invalidate.mockResolvedValue(undefined);
    ...
    const useCase = new UpdateTravelerUseCase(repository, cache, publisher);
    await useCase.execute('traveler-uuid-1', { name: 'Bob' }, 'corr-123');
  ...
  it('should return updated dto with new name', async () => {
    ...
    const useCase = new UpdateTravelerUseCase(repository, cache, publisher);
    const result = await useCase.execute('traveler-uuid-1', { name: 'Bob' });
